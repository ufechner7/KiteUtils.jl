...
function log!(logger::Logger, state::SysState)
    ...
    logger.attractor_vec[i] = copy(state.attractor)
    logger.v_app_vec[i] = state.v_app
    logger.v_wind_gnd_vec[i] = copy(state.v_wind_gnd)
    ...
    logger.vel_kite_vec[i] = copy(state.vel_kite)
    logger.acc_vec[i] = state.acc
    logger.X_vec[i] = copy(state.X)
    logger.Y_vec[i] = copy(state.Y)
    logger.Z_vec[i] = copy(state.Z)
